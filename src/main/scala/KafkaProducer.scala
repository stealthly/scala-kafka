...
case class KafkaProducer(
  ...
  def kafkaMessage(message: Array[Byte], partitionOpt: Option[Array[Byte]]): KeyedMessage[AnyRef,AnyRef] = {
    partitionOpt map { partition =>
      new KeyedMessage(topic,partition,message).asInstanceOf[KeyedMessage[AnyRef,AnyRef]]
    } getOrElse new KeyedMessage(topic, message).asInstanceOf[KeyedMessage[AnyRef, AnyRef]]
  }

  def kafkaMessages(messages: Seq[Array[Byte]])(partitionOpt: Option[Array[Byte]]): Seq[KeyedMessage[AnyRef,AnyRef]] = {
    partitionOpt map { partition =>
      messages map {message=>
        new KeyedMessage(topic,partition,message).asInstanceOf[KeyedMessage[AnyRef,AnyRef]]
      }
    } getOrElse messages map {message=>
      new KeyedMessage(topic,message).asInstanceOf[KeyedMessage[AnyRef,AnyRef]]
    }
  }

  def send(message: String): Unit = sendToPartition(message getBytes "UTF8",None)
  def sendToPartition(message: String, partitionOpt: Option[String]): Unit = sendToPartition(message getBytes "UTF8",partitionOpt map (_ getBytes "UTF8"))
  def send(message: Array[Byte]): Unit = sendToPartition(message,None)
  def sendToPartition(message: Array[Byte], partitionOpt: Option[Array[Byte]]): Unit = {
    catching(producer.send(kafkaMessage(message, partitionOpt)))
  }

  def send(messages: String*): Unit = sendToPartition(messages map (_ getBytes "UTF8"):_*)(None)
  def sendToPartition(messages: String*)(partitionOpt: Option[String]): Unit = sendToPartition(messages map (_ getBytes "UTF8"):_*)(partitionOpt map (_ getBytes "UTF8"))
  def send(messages: Array[Byte]*): Unit = sendToPartition(messages:_*)(None)
  def sendToPartition(messages: Array[Byte]*)(partitionOpt: Option[Array[Byte]]) = {
    catching(producer.send(kafkaMessages(messages)(partitionOpt):_*))
  }

  def catching(f: => Unit): Unit = try f catch { case e: Exception => e.printStackTrace() }
